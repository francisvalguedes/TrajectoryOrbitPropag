...
with st.sidebar.form("my_form"):
    index = df.columns.tolist().index(st.session_state["updated_df"].columns.tolist()[0])
    st.selectbox("Select column to delete", options=df.columns, index=index, key="delete_col")
